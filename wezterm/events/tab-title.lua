...
local _set_title = function(tab, process_name, base_title, max_width, inset)
   local title
   inset = inset or 6
   max_width = max_width or 40

   local dir_name = base_title:match("([^/]+)$")
   title = dir_name or base_title
...
M.setup = function()
   wezterm.on('format-tab-title', function(tab, _tabs, _panes, _config, hover, max_width)
      ...
      local title = _set_title(tab, process_name, tab.active_pane.title, max_width, (is_admin and 8))
